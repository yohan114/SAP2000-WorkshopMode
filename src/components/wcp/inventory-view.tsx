                      ...
                      stockItems.map((item) => {
                        ...
                        return (
                          <TableRow key={item.id} className={`hover:bg-slate-50 ${selectedIds.has(item.id) ? 'bg-emerald-50' : ''}`}>
                            ...
                            <TableCell className="text-right font-medium">${Number(item.wac).toFixed(2)}</TableCell>
                            <TableCell className="text-right font-medium">${itemValue.toLocaleString(undefined, { maximumFractionDigits: 2 })}</TableCell>
                            <TableCell>
                              <div className="flex items-center gap-2">
                    ...
                    {transactions.length === 0 ? (
                      ...
                      transactions.map((tx) => (
                        <TableRow key={tx.id} className="hover:bg-slate-50">
                          ...
                          <TableCell className="text-right font-medium">${tx.totalValue.toFixed(2)}</TableCell>
          ...
          <div className="space-y-4 py-4">
            {selectedStock && (
              <div className="p-3 bg-slate-50 rounded-lg">
                ...
                <div className="text-sm text-slate-500">WAC: ${Number(selectedStock.wac).toFixed(2)}</div>
              </div>
            )}
