...
async function seedPhotoCategories() {
  ...
  for (const category of categories) {
    await db.jcPhotoCategory.upsert({
      where: { code: category.code },
      update: { name: category.name, description: category.description, sequence: category.sequence, categoryType: category.categoryType },
      create: category,
    });
    count++;
  }
  console.log(`   ✓ Created/updated ${count} photo categories`);
}
